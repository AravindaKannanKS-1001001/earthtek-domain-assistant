...
// EarthTekniks: provider-agnostic default via env (AI_PROVIDER / AI_MODEL).
// AI_PROVIDER_API_KEY is mapped to the provider's own key var if that isn't set.
const _aiProvider = process.env.AI_PROVIDER as
  | keyof typeof staticModels
  | undefined;
const _aiModel = process.env.AI_MODEL;
if (process.env.AI_PROVIDER_API_KEY && _aiProvider) {
  const keyVar = {
    ...
    groq: "GROQ_API_KEY",
  }[_aiProvider as string];
  if (keyVar && !process.env[keyVar])
    process.env[keyVar] = process.env.AI_PROVIDER_API_KEY;
}
