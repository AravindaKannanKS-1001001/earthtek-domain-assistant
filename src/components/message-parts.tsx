...
export const ToolMessagePart = memo(
  ({
    ...
    // ── HITL: editable tool parameters ──────────────────────────────────────
    // For a gated tool the model only *proposes* arguments. The user may edit
    // them here; the approved values are what the MCP server receives.
    const editableArgs =
    ...
    }, [isManualToolInvocation, isLast, approvedArgs]);
          ...
          <div className="flex flex-col fade-in duration-300 animate-in">
            ...
            <div className="flex gap-2 py-2">
              ...
              <div className="w-full flex flex-col gap-2">
                ...
                {isManualToolInvocation && editableArgs && (
                  <div className="min-w-0 w-full p-4 rounded-lg bg-card border text-xs mt-2 flex flex-col gap-3">
                    <h5 className="text-muted-foreground font-medium">
                      Parameters — edit before running
                    </h5>
                    {Object.entries(approvedArgs ?? {}).map(([key, value]) => {
